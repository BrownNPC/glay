...
static inline Clay_Dimensions MeasureText(Clay_StringSlice text, Clay_TextElementConfig *config, uintptr_t userData) {
    ...
    Clay_Dimensions textSize = { 100};
    val ClayWasm = val::global("clay");
    val newClayStruct = ClayWasm.new_();
return  newClayStruct.call<Clay_Dimensions>("MeasureText",text,*config,userData);
...
public:
  ...
  ClayMan(const uint32_t initialWidth,
    ...
    , windowHeight(initialHeight)
  {
    ...
    Clay_Initialize(clayMemory,
                    (Clay_Dimensions){ .width = (float)windowWidth,
...
//This constructor uses a defualt measure text function
  ...
  void updateClayState(const uint32_t initialWidth,
                       const uint32_t initialHeight,
                       const float mouseX,
                       const float mouseY,
                       ...
                       const bool leftButtonDown)
  {
    ...
    Clay_UpdateScrollContainers(
      true, (Clay_Vector2){ scrollDeltaX, scrollDeltaY }, frameTime);
  }
  ...
  Clay_RenderCommandArray endLayout() { return Clay_EndLayout(); }
  ...
  void endConfig() { Clay__ElementPostConfiguration(); }
  ...
  template<size_t N>
  void textElement(const char (&text)[N],
                   const Clay_TextElementConfig textElementConfig)
  {
    Clay__OpenTextElement(
      toClayString(text),
  ...
  //A self-contained text element, with no children.
  template<size_t N>
  void textElement(const Clay_String& text,
                   const Clay_TextElementConfig textElementConfig)
  {
    Clay__OpenTextElement(
      text,
  ...
  void applyLayoutConfig(const Clay_LayoutConfig layoutConfig)
  {
    Clay__AttachLayoutConfig(Clay__StoreLayoutConfig(
      (Clay__Clay_LayoutConfigWrapper(layoutConfig)).wrapped));
  }
    ...
    Clay__AttachElementConfig(
      Clay_ElementConfigUnion{
        .rectangleElementConfig = Clay__StoreRectangleElementConfig(
          (Clay__Clay_RectangleElementConfigWrapper(rectangleConfig))
            .wrapped) },
    ...
    Clay__AttachElementConfig(
      Clay_ElementConfigUnion{
        .scrollElementConfig = Clay__StoreScrollElementConfig(
          (Clay__Clay_ScrollElementConfigWrapper{ scrollConfig }).wrapped) },
    ...
    Clay__AttachElementConfig(
      Clay_ElementConfigUnion{
        .floatingElementConfig = Clay__StoreFloatingElementConfig(
          (Clay__Clay_FloatingElementConfigWrapper{ floatingConfig })
            .wrapped) },
  ...
  {
    return { .width =
               (Clay_SizingAxis{ .size = { .minMax = { (float)w, (float)w } },
                                 .type = CLAY__SIZING_TYPE_FIXED }),
             .height =
               (Clay_SizingAxis{ .size = { .minMax = { (float)h, (float)h } },
                                 .type = CLAY__SIZING_TYPE_FIXED }) };
  }
  ...
  {
    return { .width = (Clay_SizingAxis{ .size = { .minMax = { { 0 } } },
                                        .type = CLAY__SIZING_TYPE_GROW }),
             .height = (Clay_SizingAxis{ .size = { .minMax = { { 0 } } },
                                         .type = CLAY__SIZING_TYPE_GROW }) };
  }
  ...
  {
    return {
      .width = (Clay_SizingAxis{ .size = { .minMax = { { 0 } } },
                                 .type = CLAY__SIZING_TYPE_GROW }),
    };
  }
  ...
  Clay_Padding padAll(const uint16_t p) { return { p, p, p, p }; }

  //Convenience funtion for .padding layout parameter
  Clay_Padding padX(const uint16_t p) { return { p, p, 0, 0 }; }

  //Convenience funtion for .padding layout parameter
  Clay_Padding padY(const uint16_t p) { return { 0, 0, p, p }; }
  ...
  //Convenience funtion for .padding layout parameter
  Clay_Padding padLeft(const uint16_t pl) { return { pl, 0, 0, 0 }; }

  //Convenience funtion for .padding layout parameter
  Clay_Padding padRight(const uint16_t pr) { return { 0, pr, 0, 0 }; }

  //Convenience funtion for .padding layout parameter
  Clay_Padding padTop(const uint16_t pt) { return { 0, 0, pt, 0 }; }

  //Convenience funtion for .padding layout parameter
  Clay_Padding padBottom(const uint16_t pb) { return { 0, 0, 0, pb }; }
  ...
  {
    return (Clay_String){ .length = (int32_t)str.size(),
                          .chars = insertStringIntoArena(str) };
  }
  ...
  {
    return (Clay_String){ .length = static_cast<int32_t>((N - 1)),
                          .chars = insertStringIntoArena(str) };
  }
...
private:
  ...
  void resetStringArenaIndex() { nextStringArenaIndex = 0; }
  ...
  const char* insertStringIntoArena(const std::string& str)
  {
    ...
    if (nextStringArenaIndex + strSize + 1 > maxStringArenaSize) {
      throw std::overflow_error(
        "StringArena: Not enough space to insert the string.");
    }
  ...
  void measureTime(auto start)
  {
    ...
    if (framecount % 180 == 0) {
      std::cout << "Layout completed in " << frametime << " microseconds."
                << std::endl;
    }

    if (microseconds > maxframetime) {
      maxframetime = microseconds;
      std::cout << "New maximum layout time of " << maxframetime
